...
This is a Chrome extension (Manifest V3) that lets Karim manually browse Street View,
click "Capture Sign," drag a box around a damaged sign, and save the cropped image +
metadata (matched against SIMS sign records) for later YOLO training / SIMS reporting.
Built because Google blocks headless/automated browser access to real Street View
imagery (confirmed during development — bot detection degrades coverage for
non-famous locations when driven by Playwright), so this is designed for a **human**
to drive the browser manually; the extension only automates capture + labeling.
...
degrees" input that maps any 0-359 value to NE/SE/SW/NW via a pre-built 2°-resolution
lookup table (`DEGREE_TABLE`/`cornerFromTypedDegree`), for when the auto-guessed
buttons (derived from `loc.heading` in the URL) don't match what's actually in view.
If you add another required category, follow the same pattern: track a
`selectedX` variable, add it to the `missing` array check in the save handler, and
add its field to the `record` object.
...
- `content.js` — injected on Google Maps. Floating capture button, drag-to-crop
  overlay, the Manhattan-grid compass HUD (grid runs ~29° east of true north — see
  `MANHATTAN_GRID_OFFSET`), nearest-corner SIMS matching, and the confirmation panel
  (corner picker, sign-text picker, damage category buttons).
...
This overwrites `signs_data.json` for a single zip. If Karim wants multi-zip support
later, this would need to become a dict keyed by zip, with a way to pick the active
zip in the popup — not built yet, not needed yet.

## Current capture route

Karim is working through 6th Avenue in zip 10001, north to south, tracked in
`ML Project/data/processed/capture_route_zip10001.csv` (51 corners, with
`streetview_link` per row for direct clicking). As of this note he's around W 30th St
(stop 33 of 51). Ask him for current progress rather than assuming the CSV reflects
the true state — it's a static route list, not a live tracker. Full street inventory
for zip 10001 (628 corners, 47 street names) is in
`ML Project/data/processed/signs_zip_10001.csv` if he wants a route for a different
avenue next (9th, 8th, and 7th Ave are the next-largest after 6th Ave).

Also flagged but unresolved: ~15% of corners in zip 10001 (94 of 628) have signs coded
with an `HD` suffix on `sign_code` (historic district — brown signs, not standard
...
all-caps, etc.) was written assuming standard green signs, so whether/how to apply it
to brown historic signs is an open question for Karim to raise with the DOT contacts,
not something resolved in code.
